...
def cargar_monedas(frame_lista, entry_buscar=None, label_contador=None):
    ...
    try:
        ...
        if label_contador:
            ...
            label_contador.configure(text=f"({total} moneda{'s' if total != 1 else ''})")
        ...
        for moneda in monedas:
            ...
            def crear_funcion_editar(m):
                return lambda: editar_moneda(m, frame_lista, entry_buscar, label_contador)
            
            def crear_funcion_desactivar(m):
                return lambda: desactivar_moneda_confirm(m, frame_lista, entry_buscar, label_contador)
            
            # ✅ BOTONES CON COLORES CORRECTOS
            botones = [
                ('✏️ Editar', crear_funcion_editar(moneda), COLOR_ADVERTENCIA, COLOR_ADVERTENCIA_HOVER),  # AMARILLO
                ('🗑️ Desactivar', crear_funcion_desactivar(moneda), COLOR_ERROR, COLOR_ERROR_HOVER)  # ROJO
            ]
...
def abrir_formulario_moneda(frame_principal, id_empresa, moneda_editar=None, frame_lista=None, entry_buscar=None, label_contador=None):
    ...
    entry_codigo.bind("<KeyRelease>", registrar_cambio)  # ✅ Detecta cambios
    ...
    label_ayuda = ctk.CTkLabel(
        frame_ayuda,
        text="💡 El código ISO debe tener exactamente 3 letras.\nEj: PEN (Perú), USD (Estados Unidos), EUR (Europa).",
        font=("Arial", 10),
        text_color="#666666",
        anchor="w",
        justify="left"
    ...
    def guardar_con_confirmacion():
        """Valida, confirma y guarda la moneda"""
        ...
        if es_edicion:
            respuesta = messagebox.askyesno(
                ...
                f"¿Está seguro que desea actualizar esta moneda?\n\n"
                f"💰 Código ISO: {codigo_iso}\n"
                f"📝 Nombre: {nombre}",
                parent=ventana_form
            )
        else:
            respuesta = messagebox.askyesno(
                "Confirmar Registro",
                f"¿Está seguro que desea registrar esta moneda?\n\n"
                f"💰 Código ISO: {codigo_iso}\n"
                f"📝 Nombre: {nombre}",
                parent=ventana_form
            )
    ...
    entry_nombre.bind("<Return>", lambda e: guardar_con_confirmacion())
...
def desactivar_moneda_confirm(moneda, frame_lista, entry_buscar=None, label_contador=None):
    ...
    respuesta = messagebox.askyesno(
        ...
        f"¿Está seguro de desactivar esta moneda?\n\n"
        f"💰 {codigo_iso} - {nombre}\n\n"
        f"Esta acción marcará la moneda como inactiva."
    )
