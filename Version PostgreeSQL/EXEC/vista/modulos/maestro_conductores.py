...
def cargar_conductores(frame_lista, entry_buscar=None, label_contador=None):
    ...
    try:
        ...
        if label_contador:
            ...
            label_contador.configure(text=f"({total} conductor{'es' if total != 1 else ''})")
        ...
        for conductor in conductores:
            ...
            botones = [
                ('✏️ Editar', crear_funcion_editar(conductor), COLOR_ADVERTENCIA, COLOR_ADVERTENCIA_HOVER),  # AMARILLO
                ('🗑️ Desactivar', crear_funcion_desactivar(conductor), COLOR_ERROR, COLOR_ERROR_HOVER)  # ROJO
            ]
...
def abrir_formulario_conductor(frame_principal, id_empresa, conductor_editar=None, frame_lista=None, entry_buscar=None, label_contador=None):
    ...
    entry_licencia.bind("<KeyRelease>", registrar_cambio)  # ✅ Detecta cambios
    ...
    def guardar_con_confirmacion():
        """Valida, confirma y guarda el conductor"""
        ...
        if es_edicion:
            respuesta = messagebox.askyesno(
                ...
                f"¿Está seguro que desea actualizar este conductor?\n\n"
                f"👨‍✈️ Conductor: {nombre}\n"
                f"🪪 Licencia: {licencia}",
                parent=ventana_form
            )
        else:
            respuesta = messagebox.askyesno(
                "Confirmar Registro",
                f"¿Está seguro que desea registrar este conductor?\n\n"
                f"👨‍✈️ Conductor: {nombre}\n"
                f"🪪 Licencia: {licencia}",
                parent=ventana_form
            )
    ...
    entry_licencia.bind("<Return>", lambda e: guardar_con_confirmacion())
...
def desactivar_conductor_confirm(conductor, frame_lista, entry_buscar=None, label_contador=None):
    ...
    respuesta = messagebox.askyesno(
        ...
        f"👨‍✈️ {nombre}\n"
        f"🪪 Licencia: {licencia}\n\n"
        f"Esta acción marcará al conductor como inactivo."
    )
