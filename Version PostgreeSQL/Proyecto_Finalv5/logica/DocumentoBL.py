...
class DocumentoBL:
    """
    Capa de Lógica de Negocio para Documentos de Venta.
    Maneja todas las operaciones relacionadas con boletas y facturas.
    """
    ...
    def validar_id_empresa(id_empresa) -> Tuple[bool, str, Optional[int]]:
        """
        Valida y normaliza el ID de empresa.
        
        Args:
            id_empresa: ID de empresa (puede ser int, tuple o list)
        
        Returns:
            Tuple[bool, str, Optional[int]]: (válido, mensaje, id_empresa_normalizado)
        """
        try:
            # Si es tupla o lista, extraer el entero
            if isinstance(id_empresa, (tuple, list)):
                for item in id_empresa:
                    if isinstance(item, int):
                        return True, "ID válido", item
                
                # Intentar convertir el segundo elemento o el primero
                try:
    ...
    def obtener_clientes_combo() -> List[Tuple]:
        ...
        Returns:
            List[Tuple]: Lista de tuplas (id, nombre, apellido)
        """
        db = ConexionDB()
        db.conectar()
    ...
    def obtener_formas_pago_combo() -> List[Tuple]:
        """
        Obtiene formas de pago para ComboBox.
    ...
    def obtener_monedas_combo() -> List[Tuple]:
        """
        Obtiene monedas para ComboBox.
        
        Returns:
            List[Tuple]: Lista de tuplas (id, nombre, codigo_iso)
        """
        db = ConexionDB()
        db.conectar()
    ...
    def obtener_productos_combo() -> List[Tuple]:
        """
        Obtiene productos para ComboBox.
        
        Returns:
            List[Tuple]: Lista de tuplas (id, nombre, precio, stock)
        """
        db = ConexionDB()
        db.conectar()
    ...
    def validar_carrito(carrito: List[Dict]) -> Tuple[bool, str]:
        """
        Valida que el carrito tenga productos válidos.
    ...
    def insertar_encabezado_documento(tipo_doc: str, fecha: str, id_empresa: int,
        ...
        """
        Inserta el encabezado de un documento.
        
        Returns:
            Tuple[bool, str, Optional[int]]: (éxito, mensaje, id_documento)
        """
        # Validaciones
    ...
    def insertar_detalle_documento(id_documento: int, id_producto: int, cantidad: int,
        ...
        """
        Inserta un detalle de documento.
    ...
    def crear_documento_completo(tipo_doc: str, fecha: str, id_empresa: int,
        ...
        """
        Crea un documento completo con encabezado y detalles.
        ...
        Args:
            ...
            id_forma_pago: ID de forma de pago
            id_moneda: ID de moneda
            carrito: Lista de productos [{id, nombre, cantidad, precio}]
    ...
    def obtener_documentos_empresa(id_empresa: int, filtro: str = "TODOS") -> Optional[List[Dict]]:
        """
        Obtiene documentos de una empresa con filtro opcional.
        ...
        try:
            ...
            for fila in resultados:
                documento = {
                    ...
                    'moneda': fila.get('moneda'),  # Viene como codigo_iso (CHAR(3))
    ...
    def obtener_detalle_documento(id_documento: int) -> Optional[List[Dict]]:
        """
        Obtiene el detalle de productos de un documento.
        
        Args:
            id_documento: ID del documento
        
        Returns:
            List[Dict]: Lista de productos del documento
        """
        db = ConexionDB()
        db.conectar()
