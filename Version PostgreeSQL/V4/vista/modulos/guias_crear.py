...
def abrir_formulario_guia(frame_principal, id_empresa):
    ...
    # Agregar bindings para detectar cambios en todos los campos
    def marcar_cambio(*args):
        cambios_realizados['hubo_cambios'] = True
    
    # Campos de datos de guía
    if hasattr(frame_campos, 'entry_nro_guia'):
        frame_campos.entry_nro_guia.bind("<KeyRelease>", marcar_cambio)
    if hasattr(frame_campos, 'entry_fecha_emision'):
        frame_campos.entry_fecha_emision.bind("<KeyRelease>", marcar_cambio)
    if hasattr(frame_campos, 'entry_fecha_traslado'):
        frame_campos.entry_fecha_traslado.bind("<KeyRelease>", marcar_cambio)
    if hasattr(frame_campos, 'entry_motivo'):
        frame_campos.entry_motivo.bind("<KeyRelease>", marcar_cambio)
    if hasattr(frame_campos, 'combo_documento'):
        frame_campos.combo_documento.bind("<<ComboboxSelected>>", marcar_cambio)
    ...
    # Agregar bindings adicionales a los campos de direcciones y transporte
    entry_partida.bind("<KeyRelease>", marcar_cambio)
    entry_llegada.bind("<KeyRelease>", marcar_cambio)
    combo_conductor.bind("<<ComboboxSelected>>", marcar_cambio)
    combo_vehiculo.bind("<<ComboboxSelected>>", marcar_cambio)
    
    def cancelar_con_confirmacion():
        if cambios_realizados['hubo_cambios'] or len(productos_guia) > 0:
            respuesta = messagebox.askyesno(
                "Confirmar Cancelación",
                "¿Está seguro que desea cancelar?\n\nSe perderán todos los datos ingresados y productos agregados.",
                parent=ventana_form
            )
            if respuesta:
...
def crear_seccion_datos_guia(frame_campos, id_empresa, productos_guia):
    ...
    combo_documento = ctk.CTkComboBox(
        ...
        command=lambda e: cargar_productos_documento(
            combo_documento.get(),
            documentos_dict,
            frame_campos.productos_frame,
            productos_guia
        ) if frame_campos.productos_frame else None
    )
...
def guardar_guia_completa(frame_campos, entry_partida, entry_llegada, combo_conductor,
    ...
    respuesta = messagebox.askyesno(
        ...
        f"N° Guía: {nro_guia}\n"
        f"Motivo: {motivo}\n"
        f"Productos seleccionados: {len(productos_seleccionados)}",
        parent=ventana_form
    )
