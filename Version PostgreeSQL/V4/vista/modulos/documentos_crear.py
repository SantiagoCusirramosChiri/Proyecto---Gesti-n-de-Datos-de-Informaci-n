...
def abrir_formulario_documento(frame_principal, id_empresa):
    ...
    combo_tipo.bind("<<ComboboxSelected>>", marcar_cambio)
    entry_fecha.bind("<KeyRelease>", marcar_cambio)
    combo_cliente.bind("<<ComboboxSelected>>", marcar_cambio)
    combo_forma_pago.bind("<<ComboboxSelected>>", marcar_cambio)
    combo_moneda.bind("<<ComboboxSelected>>", marcar_cambio)
    ...
    btn_agregar = ctk.CTkButton(
        frame_agregar,
        text="➕ Agregar",
        command=lambda: agregar_producto_al_carrito(
            combo_producto, entry_cantidad, productos_dict, productos_lista,
            carrito_productos, frame_carrito, ventana_form, cambios_realizados
        ),
        width=120,
        height=40,
        font=("Arial", 11, "bold"),
...
def agregar_producto_al_carrito(combo_producto, entry_cantidad, productos_dict, productos_lista,
                                carrito_productos, frame_carrito, ventana_form, cambios_realizados=None):
