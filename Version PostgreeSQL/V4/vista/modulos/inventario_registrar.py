...
def abrir_formulario_producto(frame_principal, id_empresa, producto_editar=None, callback_actualizar=None):
    ...
    combo_unidad = ctk.CTkComboBox(
        ...
        fg_color=COLOR_FONDO_TERCIARIO,
        border_color=COLOR_BORDE,
        button_color=COLOR_EXITO if not es_edicion else COLOR_ROJO_PRIMARY,
        button_hover_color=COLOR_EXITO_HOVER if not es_edicion else COLOR_ROJO_PRIMARY
    )
    ...
    label_ayuda = ctk.CTkLabel(
        frame_campos,
        text="💡 UND=Unidad, KG=Kilogramo, LT=Litro, M=Metro,\nM2=Metro cuadrado, M3=Metro cúbico",
        font=("Arial", 10),
        text_color=COLOR_TEXTO_SECUNDARIO,
        anchor="w",
        justify="left"
