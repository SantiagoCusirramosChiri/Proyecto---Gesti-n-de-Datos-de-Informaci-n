...
CREATE UNIQUE INDEX UQ_mae_empresa_RUC 
ON mae_empresa(RUC);

CREATE UNIQUE INDEX UQ_mae_conductor_licencia 
ON mae_conductor(n_licencia);

CREATE UNIQUE INDEX UQ_mae_vehiculo_placa 
ON mae_vehiculo(placa);

CREATE UNIQUE INDEX UQ_trs_encabezado_guia_nro_guia 
ON trs_encabezado_guia(nro_guia);

CREATE UNIQUE INDEX UQ_mae_producto_nombre 
ON mae_producto(nombre);

CREATE UNIQUE INDEX UQ_mae_moneda_codigo_iso 
ON mae_moneda(codigo_iso);

CREATE UNIQUE INDEX UQ_mae_identidad_codigo_documento 
ON mae_identidad(codigo_documento);


CREATE INDEX IDX_trs_encabezado_documento_fecha 
ON trs_encabezado_documento(fecha_emision);

CREATE INDEX IDX_trs_encabezado_documento_cliente 
ON trs_encabezado_documento(id_cliente);

CREATE INDEX IDX_trs_encabezado_documento_tipo 
ON trs_encabezado_documento(tipo_doc);

CREATE INDEX IDX_trs_detalle_documento_producto 
ON trs_detalle_documento(id_producto);

CREATE INDEX IDX_trs_encabezado_guia_fecha 
ON trs_encabezado_guia(fecha_emision);

CREATE INDEX IDX_trs_encabezado_guia_doc_venta 
ON trs_encabezado_guia(id_doc_venta);

CREATE INDEX IDX_mae_cliente_nombre 
ON mae_cliente(nombre);

CREATE INDEX IDX_mae_cliente_identidad 
ON mae_cliente(id_identidad);

CREATE INDEX IDX_trs_detalle_guia_producto 
ON trs_detalle_guia(id_producto);
...
BEGIN
    ...
    -- Obtener el precio del producto desde mae_producto
...
DROP TRIGGER IF EXISTS trg_recuperar_empresa;
DELIMITER $$
CREATE TRIGGER trg_recuperar_empresa
BEFORE INSERT ON mae_empresa
FOR EACH ROW
BEGIN
    DECLARE v_id INT;
    ...
    FROM mae_empresa
    WHERE RUC = NEW.RUC AND activo = FALSE
    LIMIT 1;

    IF v_id IS NOT NULL THEN
        UPDATE mae_empresa SET activo = TRUE WHERE id_empresa = v_id;
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Registro existente reactivado (empresa).';
    END IF;
END$$
DELIMITER ;
